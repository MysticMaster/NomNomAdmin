...
const postForgotPassword = async (req, res) => {
  try {
    if (!req.body) {
      return res.json({ status: 400, data: null, message: "Trống email" });
    }
    ...
    const content = {
      ...
      to: email,
      subject: `${otp} là mã xác nhận tài khoản NomNom của bạn`,
      text: `Bạn đang Lấy lại mật khẩu tài khoản ứng dụng đặt đồ ăn NomNom \nMã xác nhận là ${otp}\nVui lòng hoàn thành xác nhận trong vòng 30 phút\nNomNom\nĐây là thư hệ thống, vui lòng không trả lời thư.`,
    };
...
const autoResetPassword= async (req,res) =>{
    try {
        if (!req.body) {
          return res.json({ status: 400, data: null, message: "Trống email" });
        }
        ...
        const content = {
          ...
          text: `${newPassword} là mật khẩu mới của bạn\nNomNom\nĐây là thư hệ thống, vui lòng không trả lời thư.`,
        };
    
        try {
          await nodemailerApiController.sendMailAsync(content);
          res.json({ status: 200, data: null, message: "" });
        } catch (error) {
          res.json({
            status: 500,
            data: null,
...
const postLogin = async (req, res) => {
  try {
    ...
    if(!customer){
       return res.json({ status: 400, data: null, message: "username" });
    }

    const passwordMatch = await argon2.verify(customer.password, password);

    console.log("uusus: "+passwordMatch)
    ...
    if(!customer.status){
        return res.json({ status: 400, data: null, message: "status" });
    }
